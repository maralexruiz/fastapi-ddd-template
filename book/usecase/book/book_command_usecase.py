...
from app.domain.book import Book, BookNotFoundError, BookRepository
...
class BookCommandUseCaseUnitOfWork(ABC):
    """BookCommandUseCaseUnitOfWork defines an interface based on Unit of
    Work pattern."""
...
class BookCommandUseCase(ABC):
    """BookCommandUseCase defines a command usecase inteface related Book
    entity."""
    ...
    @abstractmethod
    def update_book(
        self, id: str, data: BookUpdateModel
...
class BookCommandUseCaseImpl(BookCommandUseCase):
    """BookCommandUseCaseImpl implements a command usecases related Book
    entity."""
    ...
    def create_book(self, data: BookCreateModel) -> Optional[BookReadModel]:
        try:
            ...
            created_book = self.uow.book_repository.create(book)
            self.uow.commit()
    ...
    def update_book(
        ...
        try:
            ...
            updated_book = self.uow.book_repository.find_by_id(id)
        except Exception:
            self.uow.rollback()
            raise
